...
YOCTO_SOURCES=${YOCTO_ROOT}/sources
...
GIT_BRANCH_LINK=https://virtium-rd0:19T31Uq2X3Kj@bitbucket.org/virtiumdevelopers/vienna.git
...
ROOT_DIR=${P2_MOUNT_DIR}/home/root
INIT_FILE=${P2_MOUNT_DIR}/etc/init.d/rc.local
function install_virtium_resources
{
	echo
	echo "Copying and installing Virtium resources"

	git clone -b ${BRANCH_NAME} ${GIT_BRANCH_LINK} 				${ROOT_DIR}/vienna	
    	cp ${YOCTO_SOURCES}/pantherascripts/imx6ul-var-dart/scripts/*.sh	${ROOT_DIR}
    	cp ${YOCTO_SOURCES}/pantherascripts/imx6ul-var-dart/config/*.conf	${ROOT_DIR}
    	cp ${YOCTO_SOURCES}/pantherascripts/imx6ul-var-dart/config/interfaces	${ROOT_DIR}

	echo "" 						  >> ${INIT_FILE}
	echo "sleep 5" 						  >> ${INIT_FILE}
	echo "chmod +x /home/root/init_led.sh" 			  >> ${INIT_FILE}
	echo "chmod +x /home/root/blink_led.sh" 		  >> ${INIT_FILE}
	echo "chmod +x /home/root/install_yocto.sh" 		  >> ${INIT_FILE}
	echo "chmod +x /home/root/turn_on_led.sh" 		  >> ${INIT_FILE}
	echo "/home/root/init_led.sh" 				  >> ${INIT_FILE}
	echo "sleep 1" 						  >> ${INIT_FILE}
	echo "/home/root/blink_led.sh &" 			  >> ${INIT_FILE}
	echo "sleep 1" 						  >> ${INIT_FILE}
	echo "/home/root/install_yocto.sh -b mx6ul -r emmc -v sd" >> ${INIT_FILE}
	echo "sleep 1" 						  >> ${INIT_FILE}
	echo "killall blink_led.sh" 				  >> ${INIT_FILE}
	echo "sleep 2" 						  >> ${INIT_FILE}
	echo "/home/root/turn_on_led.sh" 			  >> ${INIT_FILE}
	echo "" 						  >> ${INIT_FILE}
}
